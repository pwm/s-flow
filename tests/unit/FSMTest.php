...
final class FSMTest extends TestCase
{
     ...
     * @test
     */
    public function can_identify_the_event_that_failed_even_if_the_full_event_chain_is_longer(): void
    {
        $fsm = (new FSM('S1', 'S2', 'S3'))
            ->addTransition((new Transition('E1'))->from('S1')->to('S2'))
            ->addTransition((new Transition('E2'))->from('S2')->to('S3'))
            ->addTransition((new Transition('E3'))->from('S3')->to('S1'));

        // note the missing E2 thus the first E3 will fail
        $op = $fsm->deriveState('S1', 'E1', 'E3', 'E1', 'E2', 'E3');
        self::assertFalse($op->isSuccess());
        self::assertSame('S2', $op->getState());
        self::assertSame(['E1', 'E3'], $op->getEvents());
        self::assertSame('E3', $op->getLastEvent());
    }
